...
assertCampoPwa($sw !== false && str_contains($sw, "amazon-campo-v14"), 'O cache do service worker não foi atualizado.');
...
assertCampoPwa(str_contains((string)$api, "'offline_expira_em'"), 'A API não informa a validade do modo offline.');
assertCampoPwa(!str_contains((string)$api, 'campo_sessoes'), 'A API do Campo ainda depende de sessões exclusivas.');
assertCampoPwa(!str_contains((string)$api, 'campo_login_em'), 'A API do Campo ainda depende do login exclusivo.');
