...
function assinaturaAssinarRelatorio(PDO $pdo,array $input): array
{
    ...
    $pdo->beginTransaction();try{$ver=$pdo->prepare("SELECT COALESCE(MAX(versao),0)+1 FROM documento_assinaturas WHERE documento_tipo='RELATORIO' AND documento_id=:id FOR UPDATE");$ver->execute([':id'=>$id]);$versao=(int)$ver->fetchColumn();$ins=$pdo->prepare("INSERT INTO documento_assinaturas (id,documento_tipo,documento_id,versao,responsavel_id,usuario_id,assinatura_arquivo,assinatura_hash,hash_pdf_original,hash_pdf_assinado,caminho_pdf_original,caminho_pdf_assinado,token_validacao,latitude,longitude,geo_precisao_m,ip,user_agent,status,assinado_em) VALUES (:id,'RELATORIO',:doc,:versao,:resp,:usuario,:arquivo,:hashassinatura,:original,:final,:caminhooriginal,:caminhofinal,:token,:lat,:lng,:prec,:ip,:ua,'ASSINADO',:data)");$ins->execute([':id'=>$uuid,':doc'=>$id,':versao'=>$versao,':resp'=>$resp['id'],':usuario'=>$usuario,':arquivo'=>$resp['assinatura_arquivo'],':hashassinatura'=>$resp['assinatura_hash'],':original'=>$hashOriginal,':final'=>$hashFinal,':caminhooriginal'=>$dirRel.$uuid.'_original.pdf',':caminhofinal'=>$dirRel.$uuid.'.pdf',':token'=>$token,':lat'=>$lat,':lng'=>$lng,':prec'=>$prec===false?null:$prec,':ip'=>obterIpCliente(),':ua'=>substr($_SERVER['HTTP_USER_AGENT']??'',0,500),':data'=>$now->format('Y-m-d H:i:s')]);$pdo->prepare("UPDATE vistorias SET responsavel_assinatura_id=:resp,assinatura_status='ASSINADO',assinatura_em=:data WHERE id=:id")->execute([':resp'=>$resp['id'],':data'=>$now->format('Y-m-d H:i:s'),':id'=>$id]);if($cargo==='ADMIN'){$res=aprovacaoRelatorioResumoExigencias($pdo,$id);$pdo->prepare("UPDATE vistorias SET status=:status,aprovado_por=:usuario,data_aprovacao=:data WHERE id=:id")->execute([':status'=>$res['status_esperado'],':usuario'=>$usuario,':data'=>$now->format('Y-m-d H:i:s'),':id'=>$id]);if(($v['finalidade']??'VISTORIA')==='CUMPRIMENTO_EXIGENCIAS'){concluirRetornoDoRelatorio($pdo,$id);}if((int)($res['pendentes_as']??0)>0){criarPendenciaRetornoAS($pdo,$id,$usuario);}if(!empty($v['agendamento_id'])){$pdo->prepare("UPDATE ordens_servico SET status='executado' WHERE agendamento_id=:agendamento AND status IN ('pendente','em_andamento')")->execute([':agendamento'=>$v['agendamento_id']]);$pdo->prepare("UPDATE agendamentos SET status='concluido' WHERE id=:agendamento")->execute([':agendamento'=>$v['agendamento_id']]);}}$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();@unlink($originalAbs);@unlink($finalAbs);throw $e;}
    return ['token'=>$token,'validation_url'=>APP_URL.'validar-assinatura/'.$token,'admin_aprovou'=>$cargo==='ADMIN'];
}
