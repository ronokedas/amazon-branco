...
if (!function_exists('salvarImagemAssinaturaResponsavel')) {
    function salvarImagemAssinaturaResponsavel(array $arquivo, int $responsavelId): array
    {
        ...
        if (($arquivo['size'] ?? 0) < 1 || $arquivo['size'] > 2 * 1024 * 1024) throw new RuntimeException('A assinatura deve ter no máximo 2 MB.');
        ...
        if ($sourceW < 100 || $sourceH < 30) {
            imagedestroy($source);
            throw new RuntimeException('A imagem da assinatura tem resolução muito baixa.');
        }
        $canvasW = 900;
        $canvasH = 300;
        $margin = 18;
        $target = imagecreatetruecolor($canvasW, $canvasH);
        imagealphablending($target, false);
        imagesavealpha($target, true);
        $transparent = imagecolorallocatealpha($target, 255, 255, 255, 127);
        imagefill($target, 0, 0, $transparent);
        $scale = min(($canvasW - 2 * $margin) / $sourceW, ($canvasH - 2 * $margin) / $sourceH);
        $drawW = max(1, (int)round($sourceW * $scale));
        $drawH = max(1, (int)round($sourceH * $scale));
