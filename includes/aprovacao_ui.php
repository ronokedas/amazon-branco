...
function renderBotaoAprovacaoDocumento(PDO $pdo,string $tipo,string $id,string $status,bool $assinado=false,?int $responsavelId=null): void
{
    ...
    $allowed=$tipo==='RELATORIO'?in_array($status,['AGUARDANDO_APROVACAO','APROVADA','APROVADA_COM_EXIGENCIAS'],true):($tipo==='PARECER_PLANOS'?($status==='AGUARDANDO_APROVACAO'):($status==='emitido'&&!$assinado));
    if(!$allowed)return;
    $label=in_array($tipo,['RELATORIO','PARECER_PLANOS'],true)?' Aprovar e assinar':'';
...
function renderAprovacaoUi(PDO $pdo): void
{
    ...
    (function(){
      ...
      document.addEventListener('click',function(e){const btn=e.target.closest('.js-aprovar-documento');if(!btn)return;const form=btn.closest('form');const resultado=form?.querySelector('[name="resultado_relatorio"]');const versao=form?.querySelector('[name="versao_relatorio"]');if(btn.dataset.tipo==='RELATORIO'&&(!resultado?.value||!versao?.value)){window.alert('Selecione o resultado da aprovacao e atualize a pagina se necessario.');return}current=btn;if(select){select.value=btn.dataset.responsavel||'';if(!select.value&&select.options.length===2)select.selectedIndex=1}if(msg)msg.style.display='none';modal.style.display='flex'});
      ...
      confirm?.addEventListener('click',function(){if(!current)return;if(!select.value){showMessage('Selecione o responsavel tecnico.',true);return}if(!navigator.geolocation){showMessage('Este navegador nao oferece geolocalizacao.',true);return}confirm.disabled=true;showMessage('Obtendo localizacao...',false);
        navigator.geolocation.getCurrentPosition(function(pos){const fd=new FormData();const form=current.closest('form');fd.append('csrf_token',<?= json_encode(gerarCSRF()) ?>);fd.append('documento_tipo',current.dataset.tipo);fd.append('documento_id',current.dataset.id);fd.append('responsavel_id',select.value);fd.append('latitude',pos.coords.latitude);fd.append('longitude',pos.coords.longitude);fd.append('geo_precisao_m',pos.coords.accuracy||'');if(current.dataset.tipo==='RELATORIO'){fd.append('resultado_relatorio',form?.querySelector('[name="resultado_relatorio"]')?.value||'');fd.append('versao_relatorio',form?.querySelector('[name="versao_relatorio"]')?.value||'')}showMessage('Gerando e protegendo o PDF...',false);
          fetch(<?= json_encode(APP_URL.'documentos/aprovar') ?>,{method:'POST',body:fd,credentials:'same-origin'}).then(async r=>{const d=await r.json().catch(()=>({success:false,message:'Resposta invalida do servidor.'}));if(!r.ok||!d.success)throw new Error(d.message||'Falha na aprovacao.');showMessage(d.message,false);setTimeout(()=>{window.location.href=d.data.validation_url},650)}).catch(err=>{showMessage(err.message,true);confirm.disabled=false});
