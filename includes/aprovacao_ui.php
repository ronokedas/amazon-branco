...
function renderAprovacaoUi(PDO $pdo): void
{
    ...
    (function(){
      ...
      document.addEventListener('click',function(e){const btn=e.target.closest('.js-cancelar-aprovacao');if(!btn)return;const motivo=window.prompt('Informe o motivo do cancelamento. O PDF e a auditoria serao preservados:');if(!motivo||!motivo.trim())return;const fd=new FormData();fd.append('csrf_token',<?= json_encode(gerarCSRF()) ?>);fd.append('documento_tipo',btn.dataset.tipo);fd.append('documento_id',btn.dataset.id);fd.append('motivo',motivo.trim());btn.disabled=true;fetch(<?= json_encode(APP_URL.'autenticidade/cancelar') ?>,{method:'POST',body:fd,credentials:'same-origin'}).then(async r=>{const d=await r.json().catch(()=>({success:false,message:'Resposta invalida do servidor.'}));if(!r.ok||!d.success)throw new Error(d.message||'Falha no cancelamento.');window.alert(d.message);window.location.reload()}).catch(err=>{window.alert(err.message);btn.disabled=false})});
