import { CheckCircle2, CloudOff, Download, LogOut, RefreshCw, ShieldCheck, UserRound, Wifi } from 'lucide-react'
import { useState } from 'react'
import { AppShell } from '../components/AppShell'
import { BottomNav } from '../components/BottomNav'

export function SettingsScreen({ session, online, pending, syncing, onSync, onInstall, onLogout, onNavigate }) {
  const [installHelp, setInstallHelp] = useState(false)
  const instalar = async () => { if (!await onInstall?.()) setInstallHelp(true) }
  return <AppShell title="Ajustes" online={online} pending={pending} syncing={syncing} onSync={onSync} footer={false}>
    ...
    <section className="settings-groups">
      <article className="settings-card"><h2>Conexão e sincronização</h2><div className="setting-row"><span className={online ? 'setting-icon online' : 'setting-icon offline'}>{online ? <Wifi /> : <CloudOff />}</span><span><strong>{online ? 'Conectado à internet' : 'Trabalhando offline'}</strong><small>{pending ? `${pending === 1 ? '1 alteração' : `${pending} alterações`} aguardando envio` : 'Nenhuma alteração pendente'}</small></span></div><button className="primary-button" onClick={onSync} disabled={!online || syncing || !pending}><RefreshCw size={18} className={syncing ? 'spin' : ''} /> {syncing ? 'Sincronizando…' : pending ? 'Sincronizar agora' : 'Tudo sincronizado'}</button></article>
