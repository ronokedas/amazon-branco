  ...
  return (
    <AppShell title="Evidências do item" online={online} onBack={onBack}>
      ...
      <section className="evidence-content">
        ...
        {anexos.some(anexo => anexo.local) ? <div className="sync-safety-note" role="status"><strong>Foto protegida neste aparelho</strong><span>Ela será enviada junto com toda a vistoria quando você finalizar.</span></div> : null}
        {error ? <div className="form-error evidence-error" role="alert"><strong>Verifique os dados</strong><span>{error}</span></div> : null}
