import { CheckCircle2, ChevronRight, ClipboardList, CloudDownload } from 'lucide-react'
...
export function InspectionsScreen({ pacotes, online, pending, syncing, onSync, onOpen, onNavigate }) {
  const validos = pacotes.filter(item => item?.agendamento?.id)
  return <AppShell title="Vistorias baixadas" online={online} pending={pending} syncing={syncing} onSync={onSync} footer={false}>
    ...
    <section className="operational-list">
      {validos.map(pacote => {
        ...
        return <article className="operational-card" key={pacote.agendamento.id}>
          ...
          {bloqueada ? <div className="locked-state"><CheckCircle2 size={17} /> Enviada para aprovação</div> : <button className="primary-button" onClick={() => onOpen({ id: pacote.agendamento.id })}>Continuar vistoria <ChevronRight size={18} /></button>}
        </article>
      })}
