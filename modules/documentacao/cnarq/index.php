 ...
 * Listagem de Certificados Nacionais de Arqueação
 */
...
$cnarqUrl = function(array $novos = []) use (&$busca, &$filtro_status, &$paginaAtual, &$porPagina): string {
    $params = [
    ...
    return APP_URL . 'documentacao/cnarq' . ($query ? '?' . $query : '');
};
...
// Contagem total
$stmtCount = $pdo->prepare("SELECT COUNT(*) FROM certificados_cnarq c {$whereBase}");
...
$titulo_page = 'Certificados CNARQ - ' . APP_NAME;
...
<div class="conteudo-principal">
    <div class="tabela-header">
        <h2><i class="fas fa-file-certificate"></i> Certificados Nacionais de Arqueação (CNARQ)</h2>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="GET" action="<?php echo APP_URL; ?>documentacao/cnarq" class="d-flex gap-2" style="flex-wrap: wrap; align-items: flex-end;">
                <?php if ($porPagina !== 15): ?>
    ...
    <div class="tabela-container">
        <?php if (empty($certificados)): ?>
            <div class="tabela-vazia">
                ...
                <p>Nenhum certificado CNARQ encontrado.</p>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1" style="flex-wrap: nowrap;">
                                    <!-- Editar -->
                                    <a href="<?php echo APP_URL; ?>documentacao/cnarq/form?id=<?php echo h($c['id']); ?>" 
                                       class="btn btn-sm btn-primary" title="Editar">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <!-- Gerar PDF -->
                                    <a href="<?php echo APP_URL; ?>documentacao/cnarq/pdf?id=<?php echo h($c['id']); ?>" 
                                       class="btn btn-sm btn-secondary" title="Gerar PDF" target="_blank">
                                        <i class="fas fa-file-pdf"></i>
                                    </a>
                                    <?php renderBotaoAprovacaoDocumento($pdo,'CNARQ',$c['id'],$c['status'],(bool)$c['assinado']); ?>

                                    <!-- Enviar por E-mail -->
                                    <form method="POST" action="<?php echo APP_URL; ?>documentacao/cnarq/actions" 
        ...
        <div class="card-footer" style="padding: 14px 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; background: #ffffff; border-top: 1px solid #e2e8f0;">
                ...
                <div style="display: flex; align-items: center; gap: 6px;">
                    ...
                    <select id="selectPorPagina" class="form-control form-control-sm" style="width: auto; height: 32px; padding: 2px 8px; font-size: 0.82rem; border-radius: 6px; border: 1px solid #cbd5e1;" onchange="window.location.href=this.value">
                        <?php foreach ([10, 15, 25, 50, 100] as $qtd): ?>
                            <option value="<?php echo h($cnarqUrl(['por_pagina' => $qtd, 'pagina' => 1])); ?>" <?php echo $porPagina === $qtd ? 'selected' : ''; ?>>
            ...
            <?php if ($totalPaginas > 1): ?>
                <nav aria-label="Navegação de páginas de certificados CNARQ">
                    <ul class="paginacao-certificados" style="display: flex; align-items: center; gap: 5px; margin: 0; padding: 0; list-style: none;">
                        ...
                        <li class="paginacao-item <?php echo $paginaAtual <= 1 ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual <= 1 ? 'javascript:void(0)' : h($cnarqUrl(['pagina' => 1])); ?>" title="Primeira página">
                        ...
                        <!-- Página anterior -->
                        <li class="paginacao-item <?php echo $paginaAtual <= 1 ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual <= 1 ? 'javascript:void(0)' : h($cnarqUrl(['pagina' => $paginaAtual - 1])); ?>" title="Página anterior">
                        ...
                        if ($janelaInicio > 1) {
                            echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($cnarqUrl(['pagina' => 1])) . '">1</a></li>';
                            if ($janelaInicio > 2) {
                        ...
                        for ($p = $janelaInicio; $p <= $janelaFim; $p++) {
                            if ($p === $paginaAtual) {
                                ...
                                echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($cnarqUrl(['pagina' => $p])) . '">' . $p . '</a></li>';
                            }
                        }
                        ...
                        if ($janelaFim < $totalPaginas) {
                            ...
                            echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($cnarqUrl(['pagina' => $totalPaginas])) . '">' . $totalPaginas . '</a></li>';
                        }
                        ?>

                        <!-- Próxima página -->
                        <li class="paginacao-item <?php echo $paginaAtual >= $totalPaginas ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual >= $totalPaginas ? 'javascript:void(0)' : h($cnarqUrl(['pagina' => $paginaAtual + 1])); ?>" title="Próxima página">
                        ...
                        <!-- Última página -->
                        <li class="paginacao-item <?php echo $paginaAtual >= $totalPaginas ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual >= $totalPaginas ? 'javascript:void(0)' : h($cnarqUrl(['pagina' => $totalPaginas])) ?>" title="Última página">
