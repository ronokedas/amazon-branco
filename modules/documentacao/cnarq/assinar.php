...
try {
    require_once __DIR__ . '/../../../config.php';
    $stmt = $pdo->prepare("SELECT * FROM certificados_cnarq WHERE token_assinatura = :token AND ativo = 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'confirmar_assinatura') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE certificados_cnarq SET 
...
<head>
    ...
    <title>Assinar Certificado CNARQ - <?php echo h($cert['numero']); ?></title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .canvas-area h3 { color: #0d3b2e; margin-bottom: 10px; font-size: 1.1rem; }
        .canvas-wrapper {
        ...
        .btn-primary { background: #0d3b2e; color: #fff; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>Assinatura Digital - Certificado CNARQ</h1>
        ...
        <div class="footer">
            <p>Amazon Naval Ltda — Certificado Nacional de Arqueação (CNARQ)</p>
        </div>
    </div>
