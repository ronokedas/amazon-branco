...
$csnUrl = function(array $novos = []) use (&$busca, &$filtro_status, &$paginaAtual, &$porPagina): string {
    $params = [
    ...
    return APP_URL . 'documentacao/certificados' . ($query ? '?' . $query : '');
};
...
// Contagem total
$stmtCount = $pdo->prepare("SELECT COUNT(*) FROM certificados_csn c {$whereBase}");
    ...
    <div class="tabela-container">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1" style="flex-wrap: nowrap;">
                                    ...
                                    <?php renderBotaoAprovacaoDocumento($pdo,'CSN',$c['id'],$c['status'],(bool)$c['assinado']); ?>
        ...
        <div class="card-footer" style="padding: 14px 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; background: #ffffff; border-top: 1px solid #e2e8f0;">
                ...
                <div style="display: flex; align-items: center; gap: 6px;">
                    ...
                    <select id="selectPorPagina" class="form-control form-control-sm" style="width: auto; height: 32px; padding: 2px 8px; font-size: 0.82rem; border-radius: 6px; border: 1px solid #cbd5e1;" onchange="window.location.href=this.value">
                        <?php foreach ([10, 15, 25, 50, 100] as $qtd): ?>
                            <option value="<?php echo h($csnUrl(['por_pagina' => $qtd, 'pagina' => 1])); ?>" <?php echo $porPagina === $qtd ? 'selected' : ''; ?>>
            ...
            <?php if ($totalPaginas > 1): ?>
                <nav aria-label="Navegação de páginas de certificados CSN">
                    <ul class="paginacao-certificados" style="display: flex; align-items: center; gap: 5px; margin: 0; padding: 0; list-style: none;">
                        ...
                        <li class="paginacao-item <?php echo $paginaAtual <= 1 ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual <= 1 ? 'javascript:void(0)' : h($csnUrl(['pagina' => 1])); ?>" title="Primeira página">
                        ...
                        <!-- Página anterior -->
                        <li class="paginacao-item <?php echo $paginaAtual <= 1 ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual <= 1 ? 'javascript:void(0)' : h($csnUrl(['pagina' => $paginaAtual - 1])); ?>" title="Página anterior">
                        ...
                        if ($janelaInicio > 1) {
                            echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($csnUrl(['pagina' => 1])) . '">1</a></li>';
                            if ($janelaInicio > 2) {
                        ...
                        for ($p = $janelaInicio; $p <= $janelaFim; $p++) {
                            if ($p === $paginaAtual) {
                                ...
                                echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($csnUrl(['pagina' => $p])) . '">' . $p . '</a></li>';
                            }
                        }
                        ...
                        if ($janelaFim < $totalPaginas) {
                            ...
                            echo '<li class="paginacao-item"><a class="paginacao-link" href="' . h($csnUrl(['pagina' => $totalPaginas])) . '">' . $totalPaginas . '</a></li>';
                        }
                        ?>

                        <!-- Próxima página -->
                        <li class="paginacao-item <?php echo $paginaAtual >= $totalPaginas ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual >= $totalPaginas ? 'javascript:void(0)' : h($csnUrl(['pagina' => $paginaAtual + 1])); ?>" title="Próxima página">
                        ...
                        <!-- Última página -->
                        <li class="paginacao-item <?php echo $paginaAtual >= $totalPaginas ? 'disabled' : ''; ?>">
                            <a class="paginacao-link" href="<?php echo $paginaAtual >= $totalPaginas ? 'javascript:void(0)' : h($csnUrl(['pagina' => $totalPaginas])) ?>" title="Última página">
