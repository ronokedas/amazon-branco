...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'confirmar_assinatura') {
    ...
    try {
        ...
        $dir_ano = date('Y');
        $nome_arquivo_pdf = 'CNBL_' . str_replace('/', '-', $cert['numero']) . '.pdf';
        $caminho_relativo = 'storage/certificados/' . $dir_ano . '/cnbl/' . $nome_arquivo_pdf;
        ...
        if (file_exists($salvar_pdf_caminho)) {
            ...
            $stmt_pdf = $pdo->prepare("UPDATE certificados_cnbl SET caminho_arquivo_pdf = :caminho, hash_arquivo_pdf = :hash WHERE id = :id");
            $stmt_pdf->execute([
            ...
            <?php if ($cert['assinado']): ?>
                ...
                <div class="assinado-box">
                    ...
                    <div style="margin-top: 20px;">
                        <a href="<?php echo APP_URL; ?>documentacao/cnbl/pdf?token=<?php echo h($token); ?>" 
