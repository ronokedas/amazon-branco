...
function dashboardLoadData(PDO $pdo, string $cargo, string $usuarioId): array
{
    ...
    $base['acoes']=['assinadas'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM propostas p WHERE p.assinado=1 AND NOT EXISTS (SELECT 1 FROM agendamentos a WHERE a.proposta_id=p.id AND a.status<>'cancelado' AND a.data_vistoria IS NOT NULL AND a.vistoriador_id IS NOT NULL)"),'vencidas'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM agendamentos WHERE status IN ('pendente','confirmado','em_andamento') AND data_vistoria<CURDATE()"),'aprovacao'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE status='AGUARDANDO_APROVACAO'"),'emitir'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE status IN ('APROVADA','APROVADA_COM_EXIGENCIAS')")];
