...
function dashboardLoadData(PDO $pdo, string $cargo, string $usuarioId): array
{
    ...
    if ($cargo === 'VISTORIADOR') {
        ...
        $base['kpis'] = [
            ...
            'concluidas'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias v JOIN agendamentos a ON a.id=v.agendamento_id WHERE a.vistoriador_id=:uid AND v.status IN ('APROVADA','APROVADA_COM_EXIGENCIAS','REPROVADA') AND v.data_aprovacao BETWEEN DATE_FORMAT(CURDATE(),'%Y-%m-01') AND LAST_DAY(CURDATE())",$params),
        ];
        ...
        $base['historico'] = dashRows($pdo,"SELECT v.id,v.numero,v.agendamento_id,v.data_vistoria,v.status,v.finalidade,e.nome embarcacao,a.tipo_vistoria,EXISTS(SELECT 1 FROM vistorias vs WHERE vs.relatorio_anterior_id=v.id AND vs.status IN ('APROVADA','APROVADA_COM_EXIGENCIAS')) substituido FROM vistorias v JOIN agendamentos a ON a.id=v.agendamento_id JOIN embarcacoes e ON e.id=v.embarcacao_id WHERE a.vistoriador_id=:uid ORDER BY COALESCE(v.atualizado_em,v.criado_em) DESC LIMIT 6",$params);
        return $base;
    }

    if ($cargo === 'ANALISTA') {
        $base['fila'] = dashRows($pdo,"SELECT v.id,v.numero,v.agendamento_id,v.atualizado_em,e.nome embarcacao,COALESCE(u.nome,'Sem vistoriador vinculado') vistoriador,COUNT(DISTINCT va.id) fotos,COUNT(DISTINCT CASE WHEN ve.conforme='nao' THEN ve.id END) nao_conformes,TIMESTAMPDIFF(HOUR,v.atualizado_em,NOW()) horas FROM vistorias v JOIN embarcacoes e ON e.id=v.embarcacao_id LEFT JOIN agendamentos a ON a.id=v.agendamento_id LEFT JOIN usuarios u ON u.id=a.vistoriador_id LEFT JOIN vistoria_anexos va ON va.vistoria_id=v.id LEFT JOIN vistoria_exigencias ve ON ve.vistoria_id=v.id WHERE v.status='AGUARDANDO_APROVACAO' GROUP BY v.id,v.numero,v.agendamento_id,v.atualizado_em,e.nome,u.nome ORDER BY v.atualizado_em LIMIT 12");
        $base['kpis'] = ['aguardando'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE status='AGUARDANDO_APROVACAO'"),'mais_antigo'=>(int)($base['fila'][0]['horas']??0),'nao_conformes'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistoria_exigencias ve JOIN vistorias v ON v.id=ve.vistoria_id WHERE v.status='AGUARDANDO_APROVACAO' AND ve.conforme='nao'"),'concluidas_mes'=>(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE aprovado_por=:uid AND data_aprovacao BETWEEN DATE_FORMAT(CURDATE(),'%Y-%m-01') AND LAST_DAY(CURDATE())",$params)];
    ...
    $base['fluxo_aprovacoes'] = dashRows($pdo,"SELECT v.id,v.numero,v.agendamento_id,v.atualizado_em,e.nome embarcacao,COALESCE(u.nome,'Sem vistoriador') vistoriador,TIMESTAMPDIFF(HOUR,v.atualizado_em,NOW()) horas,(SELECT COUNT(*) FROM vistoria_exigencias ve WHERE ve.vistoria_id=v.id AND ve.conforme='nao') nao_conformes,(SELECT COUNT(*) FROM vistoria_anexos va WHERE va.vistoria_id=v.id) fotos FROM vistorias v JOIN embarcacoes e ON e.id=v.embarcacao_id LEFT JOIN agendamentos a ON a.id=v.agendamento_id LEFT JOIN usuarios u ON u.id=a.vistoriador_id WHERE v.status='AGUARDANDO_APROVACAO' ORDER BY v.atualizado_em ASC LIMIT 4");
    ...
    $aprovadas=(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE status IN ('APROVADA','APROVADA_COM_EXIGENCIAS')"); $analisadas=(int)dashScalar($pdo,"SELECT COUNT(*) FROM vistorias WHERE status IN ('APROVADA','APROVADA_COM_EXIGENCIAS','REPROVADA')");
    ...
    foreach ($base['vistorias_recentes'] as &$vistoriaRecente) {
        ...
        if (in_array($vistoriaRecente['status'], ['AGUARDANDO_APROVACAO', 'APROVADA', 'APROVADA_COM_EXIGENCIAS', 'REPROVADA'], true)) {
